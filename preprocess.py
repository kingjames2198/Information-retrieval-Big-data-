...
def pre_processing():
    for i in range(1, 10000):
        ...
        with open(path_out / 'doc{index}.txt'.format(index=index), 'r+', encoding='utf-8') as clfile:
            text = clfile.read()
            tokens = re.split(r"; |, |> |- |\n |_ |- |} |{ |[ |] |. |< | ", text)
            
            # Removing stop words
            stop_words = {"without", "see", "unless", "due", "also",
                        "must", "might", "like", "]", "[", "}", "{", "<", ">", "?", "\\", "\\", "/", ")", "(", "will", "may",
                        "can", "much", "every", "the", "in", "other", "this", "the", "many", "any", "an", "or", "for", "in",
                        "an", "an ", "is", "a", "about", "above", "after", "again", "against", "all", "am", "an", "and", "any",
                        ...
                        "him", "himself", "his", "how", "how’s", "i ", " i", "i’d", "i’ll", "i’m", "i’ve", "if", "in", "into",
                        "is", "isn’t", "it", "it’s", "its", "itself", "let’s", "me", "more", "most", "mustn’t", "my", "myself",
                        "no", "nor", "not", "of", "off", "on", "once", "only", "ought", "our", "ours", "ourselves", "out",
                        ...
                        "might", "like", "will", "may", "can", "much", "every", "the", "in", "other", "this", "the", "many",
                        ...
                        "Her", "Here", "Here’s", "Hers", "Herself", "Him", "Himself", "His", "How", "How’s", "I ", " I", "I’d",
            ...
            for _ in tokens:
                clean_word = "".join(list(filter(str.isalpha, _))).lower()
                clean_word = re.sub(r'[^\x00-\x7F]+,r', ' ', clean_word)
                if clean_word not in stop_words and clean_word and len(clean_word) <= 7:
                    words.update({ps.stem(clean_word): 1})

        if not os.path.exists('Indexes'):
            os.makedirs('Indexes')

        with open('Indexes/{index}.json'.format(index=int(index)), 'w', encoding='utf-8') as index_file:
            for x in sorted(dict(words)):
                index_dict[x] = [(int(index), words[x])]
